...
game.UpdateBoy = function(){
	var acc = this.acceleration, dw = this.Boy.getCurrentWidth(), dh = this.Boy.getCurrentHeight();
	if(acc != null)
	{
		//重力感应移动
		...
		if(av*dv > 0.5)
		{
			this.Boy.x -= this.Boy.currentSpeedX*1;
			if(this.Boy.x < 0) this.Boy.x = 0;
		}else if(av*dv < -0.5)
		{
			this.Boy.x += this.Boy.currentSpeedX*1;
			if(this.Boy.x > this.width - dw) this.Boy.x = this.width - dw;
		}else
		{
			this.Boy.currentSpeedX = this.Boy.speedX;
		}
	}
	else if(this.Boy.dirX != 0){
